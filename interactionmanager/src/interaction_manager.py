...
class InteractionManager:
    ...
    def __init__(self, mode, sgroups, l1, l2, concepts, cbindings, overall_round_nr, word_set, chunk_size):
        """
        ...
        #logging.log(self.LOG_LEVEL, "=== Gesture: " + json.dumps(word_set == 1) + " ===")
        ...
        #robot_gate.setGestureCondition(word_set)

        # load lesson files
        concepts = concepts.replace('.csv', '_' + str(word_set) + '.csv')
        cbindings = cbindings.replace('.csv', '_' + str(word_set) + '.csv')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default="worst", help="Value could be: 'no' or 'worst' or 'preferred'")
    ...
    parser.add_argument("--word_set", type=int, default=1, help="Word set (1 2 3)")
    parser.add_argument("--chunks", type=int, default=6, help="The size of the chunks.")

    args = parser.parse_args()
    Robot.connect(args.ip, args.port, args.sysip)
    int_manager = InteractionManager(args.mode, args.sgroups, args.L1, args.L2, args.concepts,args.cbindings, args.rnr, args.word_set, args.chunks)
    int_manager.main()
